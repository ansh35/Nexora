    ...
    return (
      <div className="min-h-screen bg-[#070B14] flex flex-col items-center justify-center p-4">
        <div className="bg-white/[0.02] border border-white/10 p-8 rounded-[24px] max-w-md w-full text-center backdrop-blur-xl">
          ...
          <p className="text-neutral-400 mb-6">No invitation token was provided.</p>
      ...
      <div className="bg-white/[0.02] border border-white/10 p-8 rounded-[24px] max-w-md w-full text-center backdrop-blur-xl relative overflow-hidden">
        ...
        {session ? (
          ...
          <div className="space-y-4">
            <Link href={`/login?callbackUrl=/invite/accept?token=${invitation.token}`} className="block w-full bg-[#22D3EE] hover:bg-[#06B6D4] text-[#070B14] font-semibold px-6 py-3 rounded-xl transition-colors">
              Log In to Accept
            </Link>
            <Link href={`/register?callbackUrl=/invite/accept?token=${invitation.token}`} className="block w-full bg-white/[0.05] hover:bg-white/[0.1] text-white font-semibold px-6 py-3 rounded-xl transition-colors">
              Create an Account
            </Link>
          </div>
        )}
